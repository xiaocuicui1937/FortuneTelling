...
class JsonParser {
    ...
    companion object {
       lateinit var mData: ResultBean
        ...
        fun loadQiu():ListBean {
            val elevenInfo = mData.十二宫信息
            val qiu = elevenInfo.酉
            val startList = qiu.星列表
            val starSmall = qiu.星庙旺
            val stars = listOf(Stars(startList.咸池, ""),
                Stars(startList.地劫, starSmall.地劫.庙旺),
                Stars(startList.天喜, ""),
                Stars(startList.天德, ""),
                Stars(startList.擎羊, starSmall.擎羊.庙旺))
            val bottomLeft = listOf(qiu.生年博士, qiu.生年将前, qiu.生年岁前)
            val bottomRight = listOf("辛巳", qiu.长生)
            val bottomCenter = listOf(qiu.宫, qiu.大限)
            val smallLimit = qiu.小限.split(",")
            val limitSb = StringBuilder()
            for (index in smallLimit.indices) {
                when (index) {
                    2 -> {
